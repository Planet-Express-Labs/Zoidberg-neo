...
from utils.paginate import progress_bar
from zoidberg.config import LL_NODES
...
class Music(commands.Cog):
    ...
    async def create_connection(self, ctx, channel=None):
        if channel is None:
            try:
                ...
                raise disnake.disnakeException('You do not appear to be in a channel I can join!')
    ...
    @commands.slash_command(name='connect',
                            ...
                            options=[
                                Option('channel', 'The channel you want the bot to join.', type=OptionType.channel)
                            ])
    async def cmd_connect_(self, ctx):
        ...
        embed = disnake.Embed(title=f'Connecting to **`{channel.name}`**',
    ...
    @commands.slash_command(name='play',
                            ...
                            options=[
                                Option('song', 'The song you want to play', type=OptionType.string, required=True)
                            ])
    ...
    @commands.slash_command(name='volume',
                            ...
                            options=[
                                Option('volume', 'What volume you want to set the bot to 0-100', type=OptionType.integer)
                            ]
                            )
    ...
    @commands.slash_command(name='now_playing',
    ...
    @commands.slash_command(name='nodes',
    ...
    async def cmd_nodes(self, ctx):
        ...
        embed = disnake.Embed(title=f"Zoidberg bot music v{__version__}", description=fmt)
        await ctx.send(embed=embed)
